...
const schema = z.object({
  ...
  employmentType: z.enum(["self_employed", "employed"]).optional().default("self_employed"),
  ...
  contactDetails: z.object({
    email: z.string().refine(val => !val || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val), { message: "Enter a valid email" }).optional().or(z.literal("")),
...
export function IdentityPage() {
  ...
  useEffect(() => {
    ...
    if (hash && ["personal", "business", "experience", "offerings", "contact", "seo"].includes(hash)) {
      setActiveTab(hash);
    } else if (!activeTab && profile.profileType) {
      setActiveTab(profile.profileType === "professional" ? "personal" : "business");
    }
  ...
  const calculateCompletion = () => {
    ...
    if (watchedValues.contactDetails?.phone || watchedValues.contactDetails?.email) score += 10;
    ...
    const hasSocials = Object.entries(socialLinks).some(([k, v]) => k !== "customLinks" && !!v);
  ...
  const toggleVisibility = () => {
    const nextMode = watchedValues.visibility === "public" ? "private" : "public";
    ...
    return (
      <div className="py-12 max-w-md mx-auto">
        <Alert variant="error" title="Profile Error">
          Unable to retrieve profile configurations. Please check connection and refresh.
        </Alert>
    ...
    <div className="space-y-8 min-w-0 select-none">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1.5">
          <span className="text-2xs uppercase tracking-[0.25em] text-brand-400 font-bold">Identity Configuration</span>
          ...
          <div className="flex items-center gap-2 pt-1">
            <span className={clsx("h-2 w-2 rounded-full", isAutosaving ? "bg-amber-400 animate-pulse" : "bg-emerald-400")} />
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          ...
          >
            {watchedValues.visibility === "public" ? "🌐 Published Live" : "🔒 Private Draft"}
          </button>
      ...
      {saveSuccess ? (
        <Alert variant="success">All settings saved successfully to your public profile.</Alert>
      ) : null}
      {saveMutation.isError ? (
        <Alert variant="error">
          {saveMutation.error?.response?.data?.message || "Failed to update profile changes. Review slug claiming handle."}
        </Alert>
      ) : null}
            ...
            <form onSubmit={onSubmit} noValidate className="space-y-6">
              ...
              <div className="flex justify-between items-center pt-6 border-t border-white/[0.05]">
                <Button
                  type="button"
                  variant="secondary"
                  disabled={currentTabList.length > 0 && activeTab === currentTabList[0].id}
          ...
          {profile?.slug ? (
            <Card className="p-5 space-y-5" hoverEffect={false}>
              <div className="border-b border-white/[0.05] pb-3.5">
                <span className="text-3xs font-bold uppercase tracking-wider text-slate-500">Live share credentials</span>
                <h4 className="text-xs font-bold text-white uppercase tracking-wider mt-0.5">Share Digital Profile</h4>
              </div>

              <div className="space-y-3 text-xs">
                {/* Copy profile url */}
                <div className="flex items-center gap-2 p-2.5 rounded-xl bg-white/[0.01] border border-white/[0.04]">
                  <span className="text-3xs font-bold text-slate-400 select-all truncate flex-1">{publicUrl}</span>
                  <Button variant="secondary" className="px-3 min-h-8 h-8 text-2xs" onClick={handleCopyLink}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  >
                    ...
                    <span className="text-3xs font-bold text-slate-400 mt-1">WhatsApp</span>
                  </a>
                  <a 
                  ...
                  >
                    <span className="text-md">💼</span>
                    <span className="text-3xs font-bold text-slate-400 mt-1">LinkedIn</span>
                  </a>
                  <a 
                  ...
                  >
                    ...
                    <span className="text-3xs font-bold text-slate-400 mt-1">Email</span>
                  </a>
                </div>

                {/* QR Code toggle action */}
                <Button variant="secondary" className="w-full text-xs" onClick={() => setShowQr(prev => !prev)}>
                ...
                <AnimatePresence>
                  {showQr ? (
                    ...
                    >
                      <img src={qrCodeUrl} alt="QR Code" className="h-36 w-36 object-contain" />
                      <a 
                        href={qrCodeUrl}

      ...
      <header className="relative z-20 border-b border-white/[0.05] bg-[#090a0f]/65 backdrop-blur-xl px-6 py-4 sticky top-0">
        <div className="mx-auto max-w-7xl flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 shrink-0">
            ...
            <span className="font-display font-extrabold text-md tracking-tight hidden xs:inline-block">OneProfile</span>
            <span className="hidden sm:inline-block h-4 w-px bg-white/10 mx-2" />
            <span className="hidden sm:inline-block text-xs text-slate-400 font-medium">Workspace</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {displayUser ? (
              <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                ...
                <span className="text-xs font-semibold text-slate-300 hidden lg:inline-block truncate max-w-[100px]">{displayUser.name}</span>
              </div>
            ) : null}
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 py-8 lg:px-8 flex-1 grid gap-8 lg:grid-cols-[240px_1fr]">
        ...
        <aside className="space-y-6">
          ...
          <div className="hidden lg:block rounded-[28px] border border-white/[0.03] bg-gradient-to-br from-white/[0.02] to-transparent p-5 backdrop-blur-sm relative overflow-hidden">
            ...
            <h4 className="text-xs font-bold text-white mb-2">Need assistance?</h4>
            <p className="text-2xs leading-relaxed text-slate-400">
              Set up your custom username, digital business card, and sync with your local integrations securely.
            </p>
            <a href="mailto:support@oneprofile.id" className="inline-block mt-3.5 text-2xs font-bold text-brand-400 hover:underline">
              Email Helpdesk →
            </a>
          </div>
        </aside>

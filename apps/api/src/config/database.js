...
function getBackoffDelay(attempt) {
  const delay =
    env.MONGODB_INITIAL_RETRY_DELAY_MS * 2 ** Math.max(0, attempt - 1);
...
async function connectTarget(uri, label, retries) {
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      ...
      // logger.info(
      //   { target: label, delay },
      //   "Waiting before next MongoDB retry",
      // );
      await sleep(delay);
    }
  }
...
export async function connectDatabase(options = {}) {
  ...
  const allowDegradedMode =
    options.allowDegradedMode ?? env.MONGODB_ALLOW_DEGRADED_MODE;
  ...
  const targets = [{ uri: env.MONGODB_URI, label: "primary" }];
